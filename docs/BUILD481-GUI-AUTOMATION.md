...
2026-09-26 用当前精确 app/codec 指纹新建 `Build481-GuiCycleSchema-20260926`。
首次打开时目标 PID、`.locked` 和资源句柄均匹配，但草稿名称栏换成两行，
原单条 OCR 名称门在保存前拒绝。校验器现只在唯一“草稿名称”标签右侧按
同列、连续行和字面精确相等拼接，负例仍拒绝错字、低置信度、跨列及外来
工程句柄。第二次试验打开及保存通过，冷重开首页首次卡片点击无响应而停止；
受控第二次点击同一目标成功。正式工具只在目标首页标题仍唯一时重新读取
AX 框并最多再点一次。第三次由已安装 Skill 正式 `gui-cycle` 完成一轮：
打开、保存、正常退出、冷启动重开及再次退出均通过，六份身份检查和四阶段
结构 `verify` 均通过，源素材 SHA 不变。随后 Skill 原生导出为 `isom`
H.264/AAC、90/90 帧、完整解码，逐帧 OCR 严格 `0…89`；详情见
`work/build481-gui-cycle-headless-regression-20260926/REPORT.md`。本次成功轮
两次卡片点击都首次成功；重试分支仅有离线契约测试，未冒称已在正式轮触发。
...
验收的官方曲线样本；这是 9 月 25 日当轮结果，曲线能力当时继续关闭。

9 月 26 日在同一隔离草稿中，实时 GUI 选择“自定义”图卡后出现可回读的青色
选中边框和五点曲线；拖动中点形成 4.375× 峰值。保存、正常退出、冷重开后，
解密材料仍保留五点与 6,166,666 µs 时长。官方 GUI 导出 185 帧 H.264/AAC，
完整解码及 185/185 帧时码 OCR 通过；该文件品牌为 `qt  `，只作为时间映射
对照。再次退出时两个标量 speed 数值被剪映调整，曲线点不变。详情见
`work/build481-curve-vip-followup-20260926/REPORT.md`。正式 Skill 曲线入口仍
失败关闭，单条 GUI 样本也不扩展 `gui-cycle` 对任意曲线控件的通用控制能力。
