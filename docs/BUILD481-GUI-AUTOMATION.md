...
**2026-09-25 更正：正式 `gui-cycle` 暂时失败关闭。** 一次 2×/60fps 隔离
GUI 对照发现，主页搜索显示目标草稿后，错误的卡片点击仍能进入另一份空草稿。
现有工具只检查编辑器时间线节点和目标草稿的磁盘 `verify`，没有独立核对
编辑器右侧实际草稿名称与绝对保存路径。因此下述历史 `verified` 报告
不能单独证明当时打开的是目标工程；必须补足界面身份回读并重新运行。
已安装 Skill 入口会在创建作业前明确拒绝，避免产生新的误报。
