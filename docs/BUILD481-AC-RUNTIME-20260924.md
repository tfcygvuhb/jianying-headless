...
**后续纠正（2026-09-25）：**上述判断只检查了帧数、完整解码和少数时码。
完整逐帧 OCR 在同类历史混速输出中发现 0.75× 段末帧落在源区间的
右开端点，1.5× 也有映射偏差候选。因此这些历史编码记录不能作为
Build 481 非 1× 速度正式开放的证据；当前正式登记和导出仅允许 1×。
独立 1× 时码样本的 90 帧均与源编号对应，详见
[速度逐帧补验](BUILD481-SPEED-20260925.md)。
