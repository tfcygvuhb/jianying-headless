...
| 仅 GUI 支持 | `gui-cycle` 对已登记、含本机资源文件的 Build 481 新建隔离工程恢复，1× 与 2× 各三轮身份回读、保存、正常退出与冷重开通过；另有保存前错误目标拒绝。工具现能识别 `jy14-edit-build/v1` 的离线记录及验证器，但 `existing_edit_publish=false` 在启动 GUI 前阻断，编辑副本未获 GUI 验收。独立 GUI 实验确认一条自定义曲线的保存、冷重开和官方导出；另一个隔离工程通过 GUI 创建复合片段、编辑子片段到 90%、冷重开并官方导出。两项均未开放正式 Skill 入口。 |
...
| 恒速 | 上游计划格式写 0.1–8x 恒速，代码将 speed 序列化；边界值可 build 不等于导出承诺。 | 历史六档 0.1/0.5/0.75/1/1.5/8x 曾构建、组合 GUI 冷重开并以抽样时间码和完整解码验收；逐帧复核又发现 8x 与计划采样映射不符，0.1/0.5x 末帧越过源范围右边界。2x 有 89/90 帧反例，90/90 文件亦每第三源帧落后一格，且不同于官方 GUI；0.75x 混速样本末帧越界，1.5x 有映射偏差候选，均缺独立单倍率对照。1x 正式出口另经 Vision 90/90 全帧验证，源标签严格 0…89；2x/60fps work-only 三次 180/180 全帧连续，官方 GUI 60fps 对照为 179/179 帧、时码 0…178。只切换 writer selector 的隔离复验改变容器品牌却仍输出 180 帧，未解释末帧差异。 | 当前仅开放 1x 正式登记和导出，所有非 1x 倍率均失败关闭。帧数与少量抽帧不足以证明逐帧正确；音质无主观验收。 |
| 曲线变速 | 原 SKILL / reference 未声明曲线变速；代码仅支持标量 speed 与线性关键帧属性，不存在 speed 曲线计划语义。 | 9 月 25 日“蒙太奇”点击未生效；9 月 26 日另一轮在隔离单片段草稿中经 GUI 选中“自定义”并拉出五点非平坦曲线。保存、正常退出、冷重开均保留曲线点与 6,166,666 µs 时长；官方 GUI 导出 H.264/AAC 185 帧、完整解码，185/185 帧源时码可读且严格递增。GUI 文件品牌为 `qt  `，仅作时间映射对照。此冷重开快照的三个自包含 work-only helper 导出均为 `isom`、185 帧、全量解码，185/185 源时码与 GUI 完全一致；一次失配沙箱作业黑帧不计入通过。导出后再次保存把标量 speed 略微改写，曲线点未变。 | GUI 精确样本和同一快照三次 helper 对照已确认；仍缺三份独立 GUI 工程、曲线计划语义、通用构建、正式 Skill 出口及主观听感，生产门禁继续关闭。 |
...
| 复合片段 | 上游有 11.4.2/11.5.0 实验代码，可离线递归检查/编辑/冻结快照导出；文档明示不能可靠交付为可编辑嵌套草稿。 | 生产 `native_compound.py` 不含 Build 481；work-only 改蓝图身份后合成图可过 strict `verify-build`，但不证明 GUI 兼容。现已在 Build 481 的独立 3 秒工程中通过官方 GUI 创建“复合片段1”：未编辑子片段时，完全退出和冷重开后的 sidecar 校验通过。进入子草稿将视频缩放设为 90% 后，GUI 保存、完全退出、再次冷重开仍在子面板显示 90%，父预览有黑边。官方 GUI 导出 H.264/AAC、90/90 帧且完整解码，实际画面保留黑边。但父草稿内嵌 child 为 90%，sidecar child 仍为 100%；`combination_id` 与 wrapper 不一致，退出后路径还丢失 child UUID 目录，严格 sidecar 校验失败。 | 仅此精确 GUI 样本可验证；Build 481 生产构建、首页登记和 Skill 原生导出仍失败关闭。须先解决 sidecar 一致性，再对独立样本重复保存、冷重开和标准 MP4 导出。证据在 `work/build481-compound-gui-20260926/`。 |
...
3. **复合片段持久化属于较大的产品能力缺口。** Build 481 官方 GUI 样本已证明单次子片段编辑可跨冷重开并导出，但父草稿、旁文件和组合 ID 在编辑后不一致，无法作为可靠的 headless 可编辑嵌套草稿。由于原文把它限定为实验能力，优先级低于已经明示恒速范围的 2× 修复。

曲线变速、贴纸、调整图层、在线模板等属于原 Skill 从未承诺的计划/API 功能；其中曲线变速已有 Build 481 官方 GUI 精确样本，但正式 Skill 入口仍关闭，不应混写为已承诺能力的回归。滤镜和花字已经主动退役。
