...
已验的[显式本地别名工具](tools/make_stix_regular_alias.py)可生成保留绑定的副本。其他字体仍保持禁用。Build 481 官方 GUI 的一条自定义曲线样本已完成保存、冷重开和 GUI 导出，但计划与正式 Skill 出口尚未验收，曲线变速及未验资源继续关闭；逐帧复核后所有非 1× 倍率均失败关闭，见[恒速补验](docs/BUILD481-SPEED-20260925.md)。叠化与轻微抖动只按固定资源身份开放，`native_resources`
总门禁仍为 false。
...
- 复合片段仅支持实验性的离线修改与冻结快照导出。Build 481 的隔离 GUI 样本虽能创建、子编辑、冷重开和官方导出，但子编辑后父草稿与旁文件的内容、组合 ID 和路径不一致；正式 Skill 构建、登记与原生导出仍失败关闭。
- 图片/GIF 样本曾出现间歇少一帧；严格帧数检查会拒绝缺帧输出，根因尚未解决。
- 不支持任意剪映版本、任意效果组合、在线模板、资源下载、云端工程或账号权益获取。
- `11.4.0 Build 481` 已开放离线新建、首页登记、检查、已有工程独立副本的离线编辑与检查、基础原生 MP4 导出、仅 1× 正式速度导出、Monaco TTF、固定 SHA 的 Arial TTF 与 STIXGeneralItalic OTF、线性关键帧、六种几何蒙版、叠化和轻微抖动；后两项仅按固定资源身份开放。编辑副本的正式登记与导出仍关闭。原始 STIXGeneral Regular OTF GUI 保存失败，固定 SHA 的本地别名已单项通过；曲线变速及未逐项验收的资源/效果仍失败关闭。
